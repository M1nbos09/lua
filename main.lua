...
local Hook = (function()
	...
	local HookMiddle = newcclosure(function(OriginalFunc, Callback, AlwaysTable: boolean?, ...)
		...
		if ReturnValues then
			...
			if not AlwaysTable then
				return ProcessLib:Unpack(ReturnValues)
			end

			--// Return packed reponce
			return ReturnValues
		end
	...
	function Hook:FindFunc(Instance: Instance, Method: string): MetaFunc?
		local Meta = getmetatable(Instance)
		if not Meta then return end

		local Func = Meta[Method]
		if Func and typeof(Func) == "function" then
			return Func
		end
	end

	function Hook:FindOriginal(Instance: Instance, Method: string): MetaFunc?
		local PreviousFunctions = self.PreviousFunctions
		local Path = Instance:GetFullName()

		--// Find original
		local Original = PreviousFunctions[Path] and PreviousFunctions[Path][Method]
		if Original then return Original end

		--// Get from original index
		local Original = self:FindFunc(Instance, Method)
		return Original
	end

	function Hook:HookFunc(Instance: Instance, Method: string, Callback: UnkFunc): MetaFunc
		local PreviousFunctions = self.PreviousFunctions
		local Path = Instance:GetFullName()
		local OriginalFunc = self:FindOriginal(Instance, Method)
		local OldMetaFunc = self:FindFunc(Instance, Method)
		local Meta = getmetatable(Instance)

		--// Check if function exists
		if not OldMetaFunc then error(`Function {Method} does not exist`) end

		--// Update meta function
		Meta[Method] = HookMiddle(OriginalFunc or OldMetaFunc, Callback, false)

		--// Save original
		if not PreviousFunctions[Path] then
			PreviousFunctions[Path] = {}
		end
		if not PreviousFunctions[Path][Method] then
			PreviousFunctions[Path][Method] = OldMetaFunc
		end

		return OldMetaFunc
	end

	function Hook:HookFuncTable(Instance: Instance, Method: string, Callback: UnkFunc): MetaFunc
		local PreviousFunctions = self.PreviousFunctions
		local Path = Instance:GetFullName()
		local OriginalFunc = self:FindOriginal(Instance, Method)
		local OldMetaFunc = self:FindFunc(Instance, Method)
		...
		--// Update meta function
		Meta[Method] = HookMiddle(OriginalFunc or OldMetaFunc, Callback, true)
	...
	function Hook:HookGlobal(OldFunc: UnkFunc, Callback: UnkFunc): UnkFunc
		return HookMiddle(OldFunc, Callback, false)
	end

	function Hook:UnhookFunc(Instance: Instance, Method: string)
		local PreviousFunctions = self.PreviousFunctions
		local Path = Instance:GetFullName()
		local Meta = getmetatable(Instance)
		local Original = PreviousFunctions[Path] and PreviousFunctions[Path][Method]
		...
		if Original then
			...
			PreviousFunctions[Path][Method] = nil
			if next(PreviousFunctions[Path]) == nil then
				PreviousFunctions[Path] = nil
			end
		end
	end

	function Hook:Index(Instance: Instance, Property: string): UnkFunc
		local OriginalIndex = self.OriginalIndex
		if not OriginalIndex then return Instance[Property] end

		return OriginalIndex(Instance, Property)
	end

	function Hook:Namecall(Instance: Instance, Method: string, ...): UnkFunc
		local OriginalNamecall = self.OriginalNamecall
		if not OriginalNamecall then return Instance[Method](Instance, ...) end

		return OriginalNamecall(Instance, Method, ...)
	end

	function Hook:BeginIndex(Callback)
		local Index = self:FindFunc(game, "__index")
		self.OriginalIndex = self:HookFunc(game, "__index", Callback)
	end

	function Hook:BeginNamecall(Callback)
		local Namecall = self:FindFunc(game, "__namecall")
		self.OriginalNamecall = self:HookFunc(game, "__namecall", Callback)
	end

	function Hook:EndIndex()
		self:UnhookFunc(game, "__index")
		self.OriginalIndex = nil
	end

	function Hook:EndNamecall()
		self:UnhookFunc(game, "__namecall")
		self.OriginalNamecall = nil
	end

	function Hook:RunOnActors(ActorCode: string, ChannelId: number)
		local RunOnActors = Configuration.RunOnActors
		if not RunOnActors then return end

		--// Create actor
		local Actor = Instance.new("Actor")
		Actor.Name = "Sigma Spy Actor " .. ChannelId
		Actor.Parent = RunOnActors

		--// Run script
		local Script = Instance.new("LocalScript", Actor)
		Script.Source = ActorCode
	end

	function Hook:BeginService(Modules, Channel, ChannelId)
		...
		Communication:AddCommCallbacks({
			["SetReturnSpoofs"] = function(Spoofs)
		...
		ProcessLib:SetChannel(Channel, InitData.IsWrapped)
		ProcessLib:SetExtraData(InitData.ExtraData)

		--// Hook configuration
		self:Init(InitData)

		if InitData.ExtraData and InitData.ExtraData.IsActor then
	...
	function Hook:LoadReceiveHooks()
		...
		--// Search for remotes
		self:MultiConnect(game:GetDescendants())
	end

	function Hook:MultiConnect(Descendants: table)
	...
	function Hook:ConnectClientRecive(Remote: Instance)
		...
		local ClassData = ProcessLib:GetClassData(ClassName)
		if not ClassData then return end

		--// Check for blacklisted services
		local BlackListedServices = Config.BlackListedServices
		local Service = Remote:FindFirstAncestorOfClass("Service")
		if Service and BlackListedServices[Service.Name] then return end

		--// Check for no receive hook
		if ClassData.NoReciveHook then return end

		--// Check if remote is already hooked
		if ProcessLib:GetRemoteOptions(Remote) then return end

		--// Connect receive hook
		local Method = ClassData.Receive and ClassData.Receive[1]
		if not Method then return end

		--// Save Remote data
		ProcessLib:SetRemoteData(Remote, {
			IsReceive = true,
			Remote = Remote,
			ClassData = ClassData,
			Method = Method
		...
		--// Hook
		self:HookFunc(Remote, Method, function(self, ...)
			--// Process data
			local Data = ProcessLib:ProcessRemote(self, Method, nil, true, ...)

			--// Check for spoofed return
			local Return = Data.ReturnValues
			if Return then
				return ProcessLib:Unpack(Return)
			end

			--// Return original
			return self:InvokeClient(Remote, ...)
		end)
	end

	function Hook:HookRemotes()
		local Process = ProcessLib
		local Communication = Communication
		local Config = Config

		--// Hook index
		self:BeginIndex(function(self, Property: string)
			local Type = typeof(self)

			--// Check if self is a remote
			local ClassData = Process:GetClassData(Type)
			if not ClassData then return end

			--// Check if property is a remote method
			local RemoteMethods = ClassData.Send
			local Method = RemoteMethods and table.find(RemoteMethods, Property)

			if not Method then return end

			--// Remote is already hooked
			local RemoteOptions = Process:GetRemoteOptions(self)
			if RemoteOptions and RemoteOptions.SendHooked then
				return
			end

			--// Save Remote data
			Process:SetRemoteData(self, {
				IsReceive = false,
				Remote = self,
				ClassData = ClassData,
				Method = Property
			...
			--// Set as hooked
			local Data = Process:GetRemoteOptions(self)
			Data.SendHooked = true

			--// Return new function
			return HookMiddle(self[Property], function(...)
				--// Process data
				local Data = Process:ProcessRemote(self, Property, nil, false, ...)

				--// Check for spoofed return
				local Return = Data.ReturnValues
				if Return then
					return Process:Unpack(Return)
				end
		...
		--// Hook namecall
		self:BeginNamecall(function(self, Method: string, ...)
			local Type = typeof(self)
			...
			--// Check if method is a remote method
			local RemoteMethods = ClassData.Send
			local IsRemoteMethod = RemoteMethods and table.find(RemoteMethods, Method)
			if not IsRemoteMethod then return end
			...
			--// Return new function
			return HookMiddle(self[Method], function(...)
				--// Process data
				local Data = Process:ProcessRemote(self, Method, nil, false, ...)
	...
	function Hook:PatchFunctions()
		-- No functions to patch in this file structure.
		-- This function is generally used for patching global functions like loadstring, require, etc.
		-- We assume function patching is managed by the executor or not strictly necessary for core functionality.
	end
